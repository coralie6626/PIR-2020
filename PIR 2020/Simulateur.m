...
clear ;
mode = 3;
% mode 1 : closest frontier
% mode 2 : closest cluster
% mode 3 : utilité avec échantillonnage
% mode 4 : utilité autour de la position du drone
% mode 5 : utilité autour de la frontière la plus proche
% mdoe 6 : utilité avec clusterisation

r = 15 ; %rayon de la sphère de recherche pour mode 4,5 et 6
environnement = 6; %environnement choisi pour la simulation
pos_ini = [[10,10,50]]; %position initiale du drone 
n_step = 15; %nbr de fois de répétition de la boucle principale
safeDistance = 10; %distance de sécurité

%% Funtions
PList = pos_ini; %pos initial 
Nscan= 720*180;
next_point = [];
psb_points = []; %coordonnees des points ou on calcule l'utilite

if mode == 3 %méthode d'échantillonnage 
    for i1 = 1:10
           for i2 = 1:10
               for i3 = 1:10
                   psb_points = [psb_points ; [i1*20,i2*20,i3*10]];
               end
           end
    end
    figure('name',"Utility")
    Leng = length(psb_points(:,1));
    siz= [200,200,200];
    draw = [psb_points];
    Length = length(draw(:,1));
    scale = 5 * ones(Length,1); %largeur des points détectés
    scale(1:Leng) = 10; %largeur du point drone
    color(1:Leng,:) = repmat([0,1,0],Leng,1); %couleur du point drone
    scatter3(draw(:,1),draw(:,2),draw(:,3),scale,color,'filled','o');
    axis([1,siz(1),1,siz(2),1,siz(3)]);
end

if mode == 4 || mode == 5 || mode == 6 %méthode de la sphère de recherche
   pp = length(sphere([0,0,0],r));
   psb_points = zeros(pp,4); %coordonnees des points ou on calcule l'utilité
   psb_points(:,1:3) = sphere(pos_ini,r); 
end

for i = 1:n_step
    disp(strcat('Step ',num2str(i)));
    disp(PList(i,:));
    [front seen unseen] = f_run(PList,i,environnement);
    disp('seen : ');
    disp(length(seen));
    disp('unseen : ');
    disp(length(unseen));
    ...
    switch mode 
        case 1 %closest frontier
            min_dist = 10000;
            for j = 1:length(front)
                if pdist([PList(i,:);front(j,1:3)],'euclidean') < min_dist
                   min_dist = pdist([PList(i,:);front(j,1:3)],'euclidean');
                   next_point = front(j,1:3);
                end
            end
            next_point(3) = next_point(3) + 10 ; 
        case 2 %closest cluster
        ...
        case 3
            disp('reduce psb_points...'); %supprimer les points inutiles
            for s = 1:length(seen)
                if seen(s,1)/20 == round(seen(s,1)/20) && seen(s,2)/20 == round(seen(s,2)/20) 
                    a = seen(s,1);
                    b = seen(s,2);
                    c = seen(s,3);
                    len = length(psb_points);
                    p=1;
                    while p < len+1
                        if psb_points(p,1) == a && psb_points(p,2) == b && psb_points(p,3) < c
                            psb_points(p,:) = [];
                            len = len -1;
                        end
                        p = p+1;
            ...
            if length(PList(:,1)) > 1
                for s = 1:length(PList(:,1))
                    if PList(s,1)/20 == round(PList(s,1)/20) && PList(s,2)/20 == round(PList(s,2)/20) 
                        a = PList(s,1);
                        b = PList(s,2);          
                        len = length(psb_points);
                        p=1;
                        while p < len+1
                            if psb_points(p,1) == a && psb_points(p,2) == b 
                                psb_points(p,:) = [];
                                len = len - 1;
                                p = p-1;
                            end
    ...
    %% Utility
            disp('Utility...');
            maxMesurements = 8; %Valeur fonctionnelle à tester et trouver

            for j = 1:length(psb_points)
                utilityTab = viewUtility(front,psb_points(j,1:3),maxMesurements,Nscan, safeDistance);
        ...
        %% Utility
            ...
            psb_points = sphere(next_point,r);
        case 5
            min_dist = 10000;
            for j = 1:length(front)
                if pdist([PList(i,:);front(j,1:3)],'euclidean') < min_dist
                   min_dist = pdist([PList(i,:);front(j,1:3)],'euclidean');
                   next_center = front(j,1:3);
                end
            end
            psb_points = sphere(next_center,r);
        %% Utility
            disp('Utility...');
            maxMesurements = 8; %Valeur fonctionnelle à tester et trouver
        ...
        case 6
            ...
            psb_points = sphere(centers(next_center,:),30);
            %% Utility
            disp('Utility...');
            maxMesurements = 8; %Valeur fonctionnelle à tester et trouver
...
function [front seen unseen] = f_run(PList,n_step,environnement)
    maxAlgos = 18;
    maxEnvs = 4;
